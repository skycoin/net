...
func TestNewUDPPendingMap(t *testing.T) {
	...
	m.AddMsg(6, &msg.Message{Seq: 5, Body: []byte{0x5}})
	t.Log(m.DelMsgAndGetLossMsgs(3))
	m.AddMsg(7, &msg.Message{Seq: 5, Body: []byte{0x5}})
	t.Log(m.DelMsgAndGetLossMsgs(6))
	m.AddMsg(8, &msg.Message{Seq: 5, Body: []byte{0x5}})
	m.AddMsg(9, &msg.Message{Seq: 5, Body: []byte{0x5}})
