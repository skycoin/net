...
//main() parses any flags and checks that the node and app keys are the correct length.
//
func main() {
	parseFlags()
	if version {
	...
	//Start new app client with type and version. This creates a new messenger factory and returns
	//a wrapper with the factory, service, apptype and version. The messengerFactory creates a UDP factory with a UDPconn
	...
	//If seed is not set on flag, gets seed from users home folder
	if !seed {
		seedPath = ""
	} else {
		if len(seedPath) < 1 {
